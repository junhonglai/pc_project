...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        const { phone, password, isAuto } = this.loginInfo;
        const { name, nickName, token } = await this.login({ phone, password });
        // console.log(re);
        // 是否自动登录
        if (isAuto) {
          localStorage.setItem(
            "user",
            JSON.stringify({ name, nickName, token })
          );
        } else {
  ...
  computed: {
    ...mapState("users", ["name", "nickname", "token"]),
  },
};
</script>
